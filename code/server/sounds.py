...
def getKeyNote(playerID, mode, instrument, keyParam):
    ...
    if instrument == INSTRUMENTS[0]:
        ...
        return getKeyNote(playerID, mode, INSTRUMENTS[randInstrument], keyParam)
...
def isValidUser(playerID):
    return len(activePlayers) > 0 and playerID in activePlayers
...
def loadKeyNotes():
    if (len(bassArray) == 0 or len(guitarArray) == 0 or len(pianoArray) == 0):
